...
import { need } from '../middleware/validate.js';
...
r.get('/buildings/:id/notices', adminOnly, (req, res) => {
...
r.post('/buildings/:id/notices', adminOnly, (req, res) => {
  assertBuildingAccess(req, req.params.id);
  ...
  audit(Number(req.params.id), 'create', 'notice', info.lastInsertRowid, req.body.title, req.user.id);
  res.status(201).json({ id: info.lastInsertRowid });
});

r.put('/notices/:id', adminOnly, (req, res) => {
  const n = loadScoped(req, (id) => db.prepare('SELECT * FROM notices WHERE id=?').get(id), req.params.id);
  ...
  db.prepare('UPDATE notices SET title=?, body=?, pinned=? WHERE id=?').run(title, body, pinned, n.id);
  audit(n.building_id, 'update', 'notice', n.id, title, req.user.id);
  res.json({ ok: true });
});

r.delete('/notices/:id', adminOnly, (req, res) => {
  const n = loadScoped(req, (id) => db.prepare('SELECT * FROM notices WHERE id=?').get(id), req.params.id);
  db.prepare('UPDATE notices SET archived=1 WHERE id=?').run(n.id);
  audit(n.building_id, 'delete', 'notice', n.id, n.title, req.user.id);
  res.json({ ok: true });
});
