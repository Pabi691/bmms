...
r.get('/buildings', asyncHandler(async (req, res) => {
  const now = new Date();
  ...
  const rows = await Promise.all(buildings.map(async (b) => {
    const admin = await db.prepare("SELECT * FROM users WHERE building_id=? AND role='building_admin'").get(b.id);
...
r.post('/buildings/:id/status', asyncHandler(async (req, res) => {
  ...
  await db.prepare('UPDATE buildings SET status=? WHERE id=?').run(req.body.status, b.id);
  if (req.body.status === 'suspended') {
    for (const u of await db.prepare('SELECT id FROM users WHERE building_id=?').all(b.id)) {
      await revokeUserSessions(u.id, 'building_suspended');
    }
  }
  await audit(b.id, 'status_change', 'building', b.id, `Building "${b.name}" set to ${req.body.status}`, req.user.id);
  res.json({ ok: true });
}));
...
r.post('/buildings/:id/admins', asyncHandler(async (req, res) => {
  const b = await db.prepare('SELECT * FROM buildings WHERE id=?').get(req.params.id);
  if (!b) throw err('Building not found', 404);
  const existing = await db.prepare("SELECT id FROM users WHERE building_id=? AND role='building_admin'").get(b.id);
  ...
  await audit(b.id, 'create', 'user', info.lastInsertRowid, `Building Admin "${username}" created for "${b.name}"`, req.user.id);
...
r.put('/users/:id', asyncHandler(async (req, res) => {
  ...
  await db.prepare("UPDATE users SET full_name=?, status=?, updated_at=datetime('now') WHERE id=?").run(fullName, status, u.id);
  if (status === 'suspended') await revokeUserSessions(u.id, 'suspended');
  await audit(u.building_id, 'update', 'user', u.id, `Building Admin "${u.username}" updated`, req.user.id);
  res.json({ ok: true });
}));
...
r.post('/users/:id/reset-password', asyncHandler(async (req, res) => {
  ...
  await audit(u.building_id, 'reset_password', 'user', u.id, `Password reset for "${u.username}"`, req.user.id);
  res.json({ password });
}));
...
r.delete('/users/:id', asyncHandler(async (req, res) => {
  ...
  await audit(u.building_id, 'delete', 'user', u.id, `Building Admin "${u.username}" deleted`, req.user.id);
  res.json({ ok: true });
}));

r.get('/users/:id/sessions', asyncHandler(async (req, res) => {
