...
r.get('/buildings/:id/residents', adminOnly, asyncHandler(async (req, res) => {
...
r.post('/flats/:id/resident-account', adminOnly, asyncHandler(async (req, res) => {
  const flat = await loadScoped(req, (id) => db.prepare('SELECT * FROM flats WHERE id=?').get(id), req.params.id);
  const existing = await db.prepare("SELECT id FROM users WHERE flat_id=? AND role='resident'").get(flat.id);
  ...
  const dupe = await db.prepare('SELECT id FROM users WHERE username=? COLLATE NOCASE').get(username);
  ...
  await audit(flat.building_id, 'create', 'user', info.lastInsertRowid, `Resident login "${username}" created for flat ${flat.number}`, req.user.id);
  const user = await db.prepare('SELECT * FROM users WHERE id=?').get(info.lastInsertRowid);
  res.status(201).json({ user: serializeUser(user), password });
}));

r.put('/flats/:id/resident-account', adminOnly, asyncHandler(async (req, res) => {
  const flat = await loadScoped(req, (id) => db.prepare('SELECT * FROM flats WHERE id=?').get(id), req.params.id);
  const u = await db.prepare("SELECT * FROM users WHERE flat_id=? AND role='resident'").get(flat.id);
  ...
  await db.prepare("UPDATE users SET status=?, updated_at=datetime('now') WHERE id=?").run(status, u.id);
  if (status === 'suspended') await revokeUserSessions(u.id, 'suspended');
  await audit(flat.building_id, 'update', 'user', u.id, `Resident login "${u.username}" set to ${status}`, req.user.id);
  res.json({ ok: true });
}));
...
r.post('/flats/:id/resident-account/reset-password', adminOnly, asyncHandler(async (req, res) => {
  ...
  const hash = await hashPassword(password);
  await db.prepare("UPDATE users SET password_hash=?, must_change_password=1, failed_login_count=0, locked_until=NULL, updated_at=datetime('now') WHERE id=?")
    .run(hash, u.id);
  await revokeUserSessions(u.id, 'password_reset');
  await audit(flat.building_id, 'reset_password', 'user', u.id, `Password reset for resident "${u.username}"`, req.user.id);
  res.json({ password });
}));

r.delete('/flats/:id/resident-account', adminOnly, asyncHandler(async (req, res) => {
  const flat = await loadScoped(req, (id) => db.prepare('SELECT * FROM flats WHERE id=?').get(id), req.params.id);
  const u = await db.prepare("SELECT * FROM users WHERE flat_id=? AND role='resident'").get(flat.id);
  if (!u) throw err('No resident login exists for this flat', 404);
  await db.prepare("UPDATE users SET status='suspended', updated_at=datetime('now') WHERE id=?").run(u.id);
  await revokeUserSessions(u.id, 'deactivated');
  await audit(flat.building_id, 'deactivate', 'user', u.id, `Resident login "${u.username}" deactivated`, req.user.id);
  res.json({ ok: true });
}));
