...
async function run() {
  const building = db.prepare("SELECT * FROM buildings WHERE name='Green Residency (Demo)'").get();
  if (!building) {
    console.log('No demo building found — run `npm run seed` first.');
    return;
  }

  const existingAdmin = db.prepare("SELECT id FROM users WHERE building_id=? AND role='building_admin'").get(building.id);
  if (!existingAdmin) {
    ...
    audit(building.id, 'create', 'user', info.lastInsertRowid, 'Demo building admin seeded', null);
  ...
  const flat = db.prepare('SELECT * FROM flats WHERE building_id=? ORDER BY id LIMIT 1').get(building.id);
  if (flat) {
    const existingResident = db.prepare("SELECT id FROM users WHERE flat_id=? AND role='resident'").get(flat.id);
    if (!existingResident) {
      ...
      audit(building.id, 'create', 'user', info.lastInsertRowid, `Demo resident seeded for flat ${flat.number}`, null);
