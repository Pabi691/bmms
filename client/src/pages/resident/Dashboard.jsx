...
import { Layout, RadialGauge, Icons, StatusChip, residentNavLinks, useToast } from '../../components/ui.jsx';
    ...
    <Layout title={`Flat ${flat.number}`} sub={`${MONTHS[now.getMonth()]} ${now.getFullYear()} · your maintenance overview`} navOverride={residentNavLinks()}
      ...
      <div className="grid hero" style={{ marginBottom: 16 }}>
        ...
        <div className="glass card-hover hero-stat">
          <div className="hs-top"><span className={'icon-badge ' + (month.due > 0 ? 'b-bad' : 'b-ok')}>{Icons.alert}</span></div>
        ...
        <div className="glass card-hover hero-stat">
          <div className="hs-top"><span className="icon-badge b-adv">{Icons.rupee}</span></div>
